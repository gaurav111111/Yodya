...
public class UserController {
	...
	public UserModel getuserById(@PathVariable Long userId, Authentication authentication) throws Exception {
		...
		return identity.get(authentication);
	}
	...
	public UpdateUserRequest updateuser(@RequestBody UpdateUserRequest updUser, @PathVariable Long userId,
		...
		cron.setEmailType(com.yupexx.bazaar.api.model.CronModel.EmailType.USER_PROFILE);
	...
	public UserModel updatePassword(@Valid @RequestBody UserChangePasswordDTO updUser, @PathVariable Long userId,
		...
		CronModel cron = new CronModel();
		cron.setVia(com.yupexx.bazaar.api.model.CronModel.Via.EMAIL);
		cron.setEmailType(com.yupexx.bazaar.api.model.CronModel.EmailType.USER_PASS);
		cron.setCreatedBy(user.getId());
		cron.setUpdatedBy(user.getId());
		cronservice.addCron(cron);
	...
	public Map<String, String> phoneVerify(Authentication authentication) throws Exception {
		System.out.println(this.getClass().getSimpleName() + " - User Email Verification is invoked.");
		...
		HashMap<String, String> map = new HashMap<>();
		map.put("message", "We have sent Phone verification request on Respected Phone Number");
